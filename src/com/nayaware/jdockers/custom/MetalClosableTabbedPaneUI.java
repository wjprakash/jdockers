...
/*
 * This is a reformatted copy of <code>javax.swing.plaf.metal.MetalTabbedPaneUI</code>
 * from JDK 1.4.2 with the following changes:
 * 
 * createUI - return an instance of the new class
 * Changed reference to BasicTabbedPaneUI.TabbedPaneLayout to 
 * BasicClosableTabbedPaneUI.TabbedPaneLayout.
 * Copied class MetalUtils with method isLeftToRight 
 *
 * @author Winston Prakash
 *  	   Stefan Matthias Aust
 * @version 1.0
 */
public class MetalClosableTabbedPaneUI extends BasicClosableTabbedPaneUI {
	...
	public static ComponentUI createUI(JComponent x) {
		return new MetalClosableTabbedPaneUI();// sma
	}
	...
	protected boolean shouldFillGap(int currentRun, int tabIndex, int x, int y) {
		...
		if (currentRun == runCount - 2) { // If it's the second to last row.
			Rectangle lastTabBounds = getTabBounds(tabPane,
	...
	protected void paintLeftTabBorder(int tabIndex, Graphics g, int x, int y,
		...
		if (tabIndex != firstIndex) {
			g.setColor(tabPane.getSelectedIndex() == tabIndex - 1 ? selectColor
		...
		if (tabIndex != firstIndex) {
			g.setColor(tabPane.getSelectedIndex() == tabIndex - 1 ? selectHighlight
					: highlight);
			g.drawLine(1, 0, 1, 4);
		}
	...
	protected void paintRightTabBorder(int tabIndex, Graphics g, int x, int y,
		...
		if (tabIndex != firstIndex) {
			g.setColor(tabPane.getSelectedIndex() == tabIndex - 1 ? tabAreaBackground
					: tabPane.getBackgroundAt(tabIndex - 1));
			g.fillRect(right - 5, 0, 5, 3);
			g.fillRect(right - 2, 3, 2, 2);
		}
		...
		if (tabPane.isOpaque()) {
			...
			case LEFT:
				g.fillRect(
						insets.left,
						insets.top,
						calculateTabAreaWidth(tabPlacement, runCount,
								maxTabWidth), size.height - insets.bottom
								- insets.top);
				break;
			case BOTTOM:
				int totalTabHeight = calculateTabAreaHeight(tabPlacement,
						runCount, maxTabHeight);
				g.fillRect(insets.left, size.height - insets.bottom
						- totalTabHeight, size.width - insets.left
						- insets.right, totalTabHeight);
				break;
			case RIGHT:
				int totalTabWidth = calculateTabAreaWidth(tabPlacement,
						runCount, maxTabWidth);
				g.fillRect(size.width - insets.right - totalTabWidth,
						insets.top, totalTabWidth, size.height - insets.top
								- insets.bottom);
				break;
			case TOP:
			default:
	...
	public class TabbedPaneLayout extends
			BasicClosableTabbedPaneUI.TabbedPaneLayout { // sma

		public TabbedPaneLayout() {
			MetalClosableTabbedPaneUI.this.super(); // sma
		}
