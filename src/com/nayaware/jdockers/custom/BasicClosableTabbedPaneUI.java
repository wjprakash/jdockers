...
/*
 * This is a reformatted copy of <code>javax.swing.plaf.basic.BasicTabbedPaneUI</code>
 * from JDK 1.4.2 with the following changes:
 * 
 * CloseButton - a new inner class representing the component's single close button
 * MouseHandler - bug fix: explicitly delegate events to parent component
 * installComponents - the close button is added to the component
 * uninstallComponents - the close button is removed again
 * getActionMap - we need to use our own private action map, not the default one
 * getFontMetrics - replaced deprecated method
 * TabbedPaneScrollLayout.layoutContainer - fixed bug that panel&button aren't hidden
 * TabbedPaneScrollLayout.layoutContainer - layout the close button
 * ScrollableTabSupport - added close button variable
 * ScrollableTabViewport - fix for wrong GTK background
 * 
 * Also removed all references to BasicGraphicsUtils.
 * 
 * Added two hooks for easier integration of GTK emulation look. 
 * 
 * @author Winston Prakash
 * 		   Stefan Matthias Aust
 * @version 1.0
 */
public class BasicClosableTabbedPaneUI extends TabbedPaneUI implements
		SwingConstants {
	 ...
	 * extended BasicClosableTabbedPaneUI to do their own layout, the UI uses
	 * the installed layoutManager (and not tabLayoutPolicy) to determine if
	...
	protected void installComponents() {
		if (scrollableTabLayoutEnabled()) {
			if (tabScroller == null) {
				...
				tabPane.add(tabScroller.closeButton); // added -sma
			}
		}
	}
	...
	protected void uninstallComponents() {
		if (scrollableTabLayoutEnabled()) {
			...
			tabPane.remove(tabScroller.closeButton); // added -sma
			tabScroller = null;
		}
	}
	...
	protected void installDefaults() {
		...
		selectedTabPadInsets = UIManager
				.getInsets("TabbedPane.selectedTabPadInsets");
		tabAreaInsets = UIManager.getInsets("TabbedPane.tabAreaInsets");
		contentBorderInsets = UIManager
	...
	private static ActionMap map; // added -sma

	ActionMap getActionMap() {
		/*
		 * removed -sma ActionMap map = (ActionMap)
		 * UIManager.get("TabbedPane.actionMap");
		 * 
		 * if (map == null) { map = createActionMap(); if (map != null) {
	 ...
	 * parameter must be a valid <code>Graphics</code> object. Tab placement may
	 * be either: <code>JTabbedPane.TOP</code>, <code>JTabbedPane.BOTTOM</code>,
	...
	protected void paintTab(Graphics g, int tabPlacement, Rectangle[] rects,
			...
			if (g instanceof Graphics2D) {
				...
				default:
					...
					if ((tabRect.x < cropline)
						...
						rect.y += yo;
						rect.height += ho;
						cropShape = createCroppedTabClip(tabPlacement, rect,
								cropline);
						cropx = cropline - 1;
						cropy = rect.y;
					}
		...
		paintTabBackground(g, tabPlacement, tabIndex, tabRect.x,
				tabRect.y + yo, tabRect.width, tabRect.height + ho, isSelected);

		paintTabBorder(g, tabPlacement, tabIndex, tabRect.x, tabRect.y + yo,
				tabRect.width, tabRect.height + ho, isSelected);
	...
	protected void paintTabBackground(Graphics g, int tabPlacement,
		...
		g.setColor(tabBackground(tabIndex, isSelected)); // modified -sma
		switch (tabPlacement) {
		case LEFT:
	...
	protected Color tabBackground(int tabIndex, boolean isSelected) {
		return !isSelected || selectedColor == null ? tabPane
	...
	protected void paintContentBorder(Graphics g, int tabPlacement,
		...
		insets = getContentBorderInsets(tabPlacement);// guards added --sma
		if (insets.top > 0)
			paintContentBorderTopEdge(g, tabPlacement, selectedIndex, x, y, w,
					h);
		if (insets.left > 0)
			paintContentBorderLeftEdge(g, tabPlacement, selectedIndex, x, y, w,
					h);
		if (insets.bottom > 0)
			paintContentBorderBottomEdge(g, tabPlacement, selectedIndex, x, y,
					w, h);
		if (insets.right > 0)
			paintContentBorderRightEdge(g, tabPlacement, selectedIndex, x, y,
					w, h);
	}
	...
	/**
	 * Returns the bounds of the specified tab index. The bounds are with
	 * respect to the JTabbedPane's coordinate space.
	 */
	...
	/**
	 * Returns the tab index which intersects the specified point in the
	 * coordinate space of the component where the tabs are actually rendered,
	 * which could be the JTabbedPane (for WRAP_TAB_LAYOUT) or a
	 * ScrollableTabPanel (SCROLL_TAB_LAYOUT).
	 */
	...
	// BasicClosableTabbedPaneUI methods
	...
	protected Icon getIconForTab(int tabIndex) {
		return (!tabPane.isEnabled() || !tabPane.isEnabledAt(tabIndex)) ? tabPane
				.getDisabledIconAt(tabIndex) : tabPane.getIconAt(tabIndex);
	}

	/**
	 * Returns the text View object required to render stylized text (HTML) for
	 * the specified tab or null if no specialized text rendering is needed for
	...
	protected int calculateTabAreaHeight(int tabPlacement, int horizRunCount,
		...
		return (horizRunCount > 0 ? horizRunCount
				* (maxTabHeight - tabRunOverlay) + tabRunOverlay
	...
	protected int calculateTabAreaWidth(int tabPlacement, int vertRunCount,
		...
		Insets tabAreaInsets = getTabAreaInsets(tabPlacement);
		int tabRunOverlay = getTabRunOverlay(tabPlacement);
		return (vertRunCount > 0 ? vertRunCount * (maxTabWidth - tabRunOverlay)
				+ tabRunOverlay + tabAreaInsets.left + tabAreaInsets.right : 0);
	}
	...
	protected FontMetrics getFontMetrics() {
		/*
		 * removed -sma Font font = tabPane.getFont(); return
		 * Toolkit.getDefaultToolkit().getFontMetrics(font);
		 */
		return tabPane.getFontMetrics(tabPane.getFont()); // added -sma
	}
	...
	protected void selectAdjacentRunTab(int tabPlacement, int tabIndex,
		...
		case LEFT:
		case RIGHT:
			newIndex = getTabAtLocation(r.x + r.width / 2 + offset, r.y
					+ r.height / 2);
			break;
		case BOTTOM:
		case TOP:
		default:
			newIndex = getTabAtLocation(r.x + r.width / 2, r.y + r.height / 2
					+ offset);
		}
		if (newIndex != -1) {
	...
	protected int getTabRunOffset(int tabPlacement, int tabCount, int tabIndex,
		...
		case LEFT: {
			if (run == 0) {
				offset = (forward ? -(calculateTabAreaWidth(tabPlacement,
						runCount, maxTabWidth) - maxTabWidth) : -maxTabWidth);

			} else if (run == runCount - 1) {
				offset = (forward ? maxTabWidth : calculateTabAreaWidth(
						tabPlacement, runCount, maxTabWidth) - maxTabWidth);
			} else {
		...
		case BOTTOM: {
			if (run == 0) {
				offset = (forward ? maxTabHeight : calculateTabAreaHeight(
						tabPlacement, runCount, maxTabHeight) - maxTabHeight);
			} else if (run == runCount - 1) {
				offset = (forward ? -(calculateTabAreaHeight(tabPlacement,
						runCount, maxTabHeight) - maxTabHeight) : -maxTabHeight);
			} else {
	...
	boolean requestFocusForVisibleComponent() {
		...
		if (visibleComponent.isFocusable()) {
			...
			Container c1 = (JComponent) visibleComponent;
			Component c2 = c1.getFocusTraversalPolicy().getDefaultComponent(c1);
	...
	private static class RightAction extends AbstractAction {
		public void actionPerformed(ActionEvent e) {
			...
			BasicClosableTabbedPaneUI ui = (BasicClosableTabbedPaneUI) pane
					.getUI();
	...
	private static class SetSelectedIndexAction extends AbstractAction {
		public void actionPerformed(ActionEvent e) {
			...
			if (pane != null
					&& (pane.getUI() instanceof BasicClosableTabbedPaneUI)) {
				BasicClosableTabbedPaneUI ui = (BasicClosableTabbedPaneUI) pane
						.getUI();
	 ...
	 * Instantiate it only within subclasses of BasicClosableTabbedPaneUI.
	 */
	public class TabbedPaneLayout implements LayoutManager {
		...
		protected Dimension calculateSize(boolean minimum) {
			...
			for (int i = 0; i < tabPane.getTabCount(); i++) {
				...
				if (component != null) {
					...
					size = minimum ? component.getMinimumSize() : component
							.getPreferredSize();
		...
		protected int preferredTabAreaWidth(int tabPlacement, int height) {
			...
			if (tabCount > 0) {
				...
				for (int i = 0; i < tabCount; i++) {
					int tabHeight = calculateTabHeight(tabPlacement, i,
							fontHeight);
				...
				total = calculateTabAreaWidth(tabPlacement, columns,
						maxTabWidth);
			}
			return total;
		}
		...
		public void layoutContainer(Container parent) {
			...
			if (selectedIndex < 0) {
				...
				if (numChildren > 0) {
					...
					cw = bounds.width - totalTabWidth - insets.left
							- insets.right - contentInsets.left
							- contentInsets.right;
		...
		protected void calculateTabRects(int tabPlacement, int tabCount) {
			...
			boolean leftToRight = tabPane.getComponentOrientation()
					.isLeftToRight();
			...
			if (runCount > 1) {
				...
				normalizeTabRuns(tabPlacement, tabCount, verticalTabRuns ? y
						: x, returnAt);
		...
		protected void padTabRun(int tabPlacement, int start, int end, int max) {
			...
			if (tabPlacement == TOP || tabPlacement == BOTTOM) {
				...
				for (int j = start; j <= end; j++) {
					...
					pastRect.width += Math.round((float) pastRect.width
							* factor);
				}
				...
				for (int j = start; j <= end; j++) {
					...
					pastRect.height += Math.round((float) pastRect.height
							* factor);
				}
	...
	private class TabbedPaneScrollLayout extends TabbedPaneLayout {
		...
		public void layoutContainer(Container parent) {
			...
			if (selectedIndex < 0) {
				...
				// added -sma
				tabScroller.tabPanel.setVisible(false);
				tabScroller.viewport.setVisible(false);
				tabScroller.scrollBackwardButton.setVisible(false);
				tabScroller.scrollForwardButton.setVisible(false);
				tabScroller.closeButton.setVisible(false);
				//
			} else {
				// added -sma
				tabScroller.tabPanel.setVisible(true);
				tabScroller.viewport.setVisible(true);
				//
				...
				boolean closable = ((JClosableTabbedPane) tabPane)
						.isCloasable();// added --sma
				...
				if (numChildren > 0) {
					...
					for (int i = 0; i < numChildren; i++) {
						...
						if (child instanceof ScrollableTabViewport) {
							...
							case RIGHT:
								int totalTabHeight = rects[tabCount - 1].y
										+ rects[tabCount - 1].height;
								if (totalTabHeight > th) {
									int dir = scrollbutton.scrollsForward() ? SOUTH
											: NORTH;
									scrollbutton.setDirection(dir);
									visible = true;
									bx = (tabPlacement == LEFT ? tx + tw
											- bsize.width : tx);
									by = dir == SOUTH ? bounds.height
						...
						} else if (child instanceof CloseButton) { // added -sma
	...
	public class CloseButton extends BasicArrowButton implements UIResource {
		public CloseButton() {
			// we cannot use the BasicArrowButton's color variables so we
			// don't bother setting them. Instead, we use the TabPaneUI's
			// color variables which are initialized similar. Please notice
			// that we have to use "lightHighlight" instead of "highlight."
			super(0, UIManager.getColor("TabbedPane.selected"), UIManager
					.getColor("TabbedPane.shadow"), UIManager
					.getColor("TabbedPane.darkShadow"), UIManager
					.getColor("TabbedPane.highlight"));
		}

		/* Overwritten to draw a cross instead of an arrow */
		public void paintTriangle(Graphics g, int x, int y, int size,
				int direction, boolean isEnabled) {
			size += 2;
			Color oldColor = g.getColor();
			for (int i = 0; i < 2; i++) {
				g.setColor(isEnabled ? darkShadow : i == 0 ? lightHighlight
						: shadow);
				x--;
				y--;
				if (isEnabled && i == 0) {
				...
				g.drawLine(x + 1, y, x + size, y + size - 1);
		...
		/* Overwritten to delegate the action event */
		protected void fireActionPerformed(ActionEvent event) {
			super.fireActionPerformed(event);
			((JClosableTabbedPane) tabPane).fireCloseAction();
		}
	}
	...
	private class ScrollableTabSupport implements ChangeListener {
		...
		public CloseButton closeButton = new CloseButton(); // added -sma
		...
		ScrollableTabSupport(int tabPlacement) {
			...
			// added -sma
			hookButtons(scrollForwardButton, scrollBackwardButton, closeButton);
		}
	...
	private class ScrollableTabViewport extends JViewport implements UIResource {
		...
		// looks like that in GTK Look and Feel, a JViewport has always a white
		// background
		// overwriting this method fixes the problem -sma
	...
	private class ScrollableTabPanel extends JPanel implements UIResource {
		...
		public void paintComponent(Graphics g) {
			super.paintComponent(g);
			BasicClosableTabbedPaneUI.this.paintTabArea(g,
					tabPane.getTabPlacement(), tabPane.getSelectedIndex());
		}
	}

	private class ScrollableTabButton extends BasicArrowButton implements
	...
	public class PropertyChangeHandler implements PropertyChangeListener {
		public void propertyChange(PropertyChangeEvent e) {
			...
			if ("mnemonicAt".equals(name)) {
				...
				BasicClosableTabbedPaneUI.this.uninstallUI(pane);
				BasicClosableTabbedPaneUI.this.installUI(pane);
			} else if (name.equals("closable")) { // added --sma
				pane.doLayout();
			} else if (name.equals("contentBorder")) { // added --sma
	...
	public class MouseHandler extends MouseAdapter implements
			MouseMotionListener {
		// added -sma
		private boolean grabMotionEvents;

		private void delegate(MouseEvent e) {
			if (scrollableTabLayoutEnabled()) {
				if (!grabMotionEvents) {
					grabMotionEvents = true;
		...
		public void mouseExited(MouseEvent e) {
			if (!tabPane.getBounds().contains(e.getX(), e.getY())) {
				delegate(e);
			}
		}
